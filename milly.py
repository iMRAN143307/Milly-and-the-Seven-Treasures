...
async def main():
    ...
    for i, pos in enumerate(enemy_spawns):
        enemies.append({
            ...
            "vy": 0.0,
            "angle": 0.0,
            "img": enemy_images[i % len(enemy_images)],
            "trail": [],
            "grazed": False
        })
    ...
    intro_frame_index = 0
    intro_timer = 0
    TIME_PER_FRAME = 180 # Reduced from 300 to 100 for a much faster framerate
    ...
    while running:
        ...
        for enemy in enemies:
            ...
            if -100 < screen_ex < 1280 + 100 and -100 < screen_ey < 720 + 100:
                angle_deg = math.degrees(enemy["angle"])
                rotated_enemy = pygame.transform.rotate(enemy["img"], -angle_deg - 90)
        ...
        if nearest_target:
            ...
            rotated_arrow = pygame.transform.rotate(arrow_img, snapped_angle)
